...
const Portfolio = () => {
  ...
  const labelPosition = isEven ? 'left-0 pr-[4vw] sm:pr-[5vw] md:pr-[6vw]' : 'left-1/2 pl-[4vw] sm:pl-[5vw] md:pl-[6vw]';
  ...
  return (
    ...
    >
      ...
      {/* Info Box */}
      <div className="fixed top-100 hidden lg:block md:block lg:top-60 left-4 z-40 bg-slate-800/90 backdrop-blur-xl border border-indigo-500/30 rounded-xl p-3 sm:p-4 text-slate-300 max-w-[85vw] sm:max-w-xs md:max-w-sm shadow-lg">
        ...
        <ul className="space-y-1 text-[11px] sm:text-sm">
          {[
            ...
            <li key={i} className="flex items-center gap-2">
              <div className={clsx('w-2 h-2 rounded-full flex-shrink-0', item.color)} />
              {item.label}
            </li>
          ))}
        </ul>
